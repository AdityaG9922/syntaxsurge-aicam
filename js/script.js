...
window.exportPDF = function() {
  const previewBox = document.getElementById('pdf-preview');
  if (!previewBox || !previewBox.classList.contains('show')) {
    showToast('Upload an image first'); return;
  }
  const img = previewBox.querySelector('img');
  ...
  function doExport() {
    ...
    imgEl.onload = function() {
      const ratio = Math.min((pdfW - 20) / imgEl.width, (pdfH - 40) / imgEl.height);
      const w = imgEl.width * ratio; const h = imgEl.height * ratio;
      canvas.width = imgEl.width; canvas.height = imgEl.height;
      canvas.getContext('2d').drawImage(imgEl, 0, 0);
      const imgData = canvas.toDataURL('image/jpeg', 0.92);
      const x = (pdfW - w) / 2; const y = 20;
      pdf.addImage(imgData, 'JPEG', x, y, w, h);
      pdf.setFontSize(8); pdf.setTextColor(100);
      pdf.text('Generated by Syntax Surge · syntax-surge.app', pdfW / 2, pdfH - 8, { align: 'center' });
      pdf.save('syntax-surge-scan.pdf');
      if (btn) { btn.disabled = false; btn.textContent = 'Export PDF'; }
      showToast('PDF downloaded');
      const rp = document.getElementById('pdf-result');
      if (rp) rp.classList.add('show');
    };
    imgEl.onerror = () => { showToast('Image load error'); if (btn) { btn.disabled = false; btn.textContent = 'Export PDF'; } };
    imgEl.src = img.src;
  }
